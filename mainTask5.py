...
cwd = os.getcwd()
print cwd
with open(os.path.join(cwd, 'dataset', 'task5', 'train', 'dataset_train.txt')) as data_file:
    train_data = json.load(data_file)
with open(os.path.join(cwd, 'dataset', 'task5', 'dev', 'dataset_dev.txt')) as data_file:
    development_data = json.load(data_file)
with open(os.path.join(cwd, 'dataset', 'task5', 'test', 'dataset_test.txt')) as data_file:
...
if True:
    for idd in train_data:
        tmp = train_data[idd]["atext_clear_lower"].split(" ")
        for word in tmp:
            bag_of_words(word)
...
if True:
  ...
  for train_part in [train_data]:
    ...
    for idd in train_part:
        train_tweet_json = train_part[idd]
        ...
        #sys.exit(0)
        train_x.append(tmp)
        train_y.append(train_tweet_json['classes'])
      #except:
  ...
  for idd in test_data:
        tweet_json = test_data[idd]
        ...
        test_x[0].append(tmp)
        test_y[0].append(tweet_json['classes'])
        ...
        test_x[1].append(tmp)
        test_y[1].append(tweet_json['classes'])
        ...
        test_x[2].append(tmp)
        test_y[2].append(tweet_json['classes'])
        ...
        test_x[3].append(tmp)
        test_y[3].append(tweet_json['classes'])
        ...
        test_x[4].append(tmp)
        test_y[4].append(tweet_json['classes'])
        ...
        test_x[5].append(tmp)
        test_y[5].append(tweet_json['classes'])
        ...
        test_x[6].append(tmp)
        test_y[6].append(tweet_json['classes'])
        ...
        test_x[7].append(tmp)
        test_y[7].append(tweet_json['classes'])
        ...
        test_x[8].append(tmp)
        test_y[8].append(tweet_json['classes'])
        ...
        test_x[9].append(tmp)
        test_y[9].append(tweet_json['classes'])
        ...
        test_y[10].append(tweet_json['classes'])
        ...
        test_y[11].append(tweet_json['classes'])
  ...
  classes_names = ["anger", "anticipation", "disgust", "fear", "joy", "love", "optimism", "pessimism", "sadness",
                   "surprise", "trust"]
  ...
  for emotion_index in range(11):
      print classes_names[emotion_index] + '~' * 50
      data_tmp = [[], []]
      classes_tmp = [[], []]
      for index in range(train_size):
              label = 0
              if emotion_index in train_y[index]:
                  label = 1
              data_tmp[0].append(train_x[index])
              classes_tmp[0].append(label)
      data_tmp[1] = test_x

      #clf.fit(data_tmp[0], classes_tmp[0])
      #predicted = clf.predict(data_tmp[1])

      print 'pred before' + '-' * 50
      p = prediction(data_tmp[0], classes_tmp[0], data_tmp[1])
      pppp.append(p)
      print 'pred after' + '=' * 50

  '''
  text_for_prediction_file = 'ID	Tweet	anger	anticipation	disgust	fear	joy	love	optimism	pessimism	sadness	surprise	trust\n'
  iiii = 0
  for idd in test_data:
      new_text = (idd + '\t' + test_data[idd]['atext'])
      tmp = ""
      for i in range(11):
          tmp += '\t' + str(pppp[i][iiii])
      iiii += 1
      text_for_prediction_file += new_text + tmp + '\n'
  f_prediction = open(os.path.join(cwd, 'dataset', 'task5', 'submission', 'E-C_en_pred.txt'), 'w+')
