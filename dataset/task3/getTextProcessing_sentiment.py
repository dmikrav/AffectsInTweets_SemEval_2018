...
# In[35]:
folder_file_name = "test"
with open(os.path.join(cwd, folder_file_name, "dataset_json_" + folder_file_name + ".txt")) as data_file:
  ...
  for idd in data[aff]:
        ...
        if not ('metrics' in data[aff][idd] and 'textProcessingCom' in data[aff][idd]['metrics']):
          try:
            ...
            excepted_total += 1
        i += 1
        print i, done, excepted, excepted_total
            #get_json(item['text'])
...
f = open(os.path.join(cwd, folder_file_name, "dataset_json_" + folder_file_name + "_2.txt"), 'w+')
f.write(a)
f.close()
